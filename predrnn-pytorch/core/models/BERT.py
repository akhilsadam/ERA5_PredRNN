...
class BERT(BaseModel):
    ...
    def __init__(self, num_layers, num_hidden, configs, preprocessor=None, model_args=None):
        ...
        assert configs.input_length == configs.total_length//2, "BERT model requires input_length == total_length//2"
        
        self.model = BERT_base(self,
                         ntoken=configs.input_length,
                         ninp=preprocessor.latent_dims[-1],
                         nhead=self.model_args['n_head'],
                         nhid=self.model_args['n_embd'],
