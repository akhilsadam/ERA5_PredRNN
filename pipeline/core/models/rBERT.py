...
from core.loss import loss_mixed #, loss_perceptual 
...
class rBERT(BaseModel):
    ...
    def __init__(self, num_layers, num_hidden, configs):
        super(rBERT, self).__init__(num_layers, num_hidden, configs)
        ...
        self.nstep = 8 if 'nstep' not in self.model_args else self.model_args['nstep']
        # transformer
    ...
    def core_forward(self, seq_total, istrain=True, **kwargs):
        ...
        loss_pred = loss_mixed(out, seq_total, self.input_length, self.weight) #+ 0.1*loss_perceptual(out, seq_total, self.input_length)
