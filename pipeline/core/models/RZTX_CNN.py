...
class RZTX_CNN(BaseModel):
    ...
    def __init__(self, num_layers, num_hidden, configs):
        ...
        self.model = ReZero_base( \
                         ntoken=ntoken,
                         channels = self.preprocessor.latent_dims[-1],
                         ninp=ninp,
...
class CNN(nn.Module):
    def __init__(self, k1, k2, channels, spatial) -> None:
        ...
        if spatial:
            ...
            self.seq = nn.Sequential(self.pad_1x, self.pad_1y, self.conv1, nn.ReLU(), self.pad_2x, self.pad_2y, self.conv2, nn.ReLU())
            ...
            self.seq = nn.Sequential(self.conv1, nn.ReLU(), self.conv2, nn.ReLU())
            
            
class CNNAutoencoder(nn.Module):
    def __init__(self, k1, k2, channels) -> None:
        super().__init__()
        assert k1%2==1 and k2%2==1, "Kernel sizes must be odd!"
        ...
        self.conv2 = nn.Conv2d(channels, channels, (k2, k2), dilation=(4,4)) 
        self.conv3 = nn.Conv2d(channels, channels, (k2, k2), dilation=(4,4)) # 
                
        
        self.encode = nn.Sequential(self.pad_1x, self.pad_1y, self.conv1, self.conv2, self.conv3)
            
        # decoder
        self.rconv3 = nn.ConvTranspose2d(channels, channels, (k2, k2), dilation=(4,4)) #
        self.rconv2 = nn.ConvTranspose2d(channels, channels, (k2, k2), dilation=(4,4)) #
        self.rconv1 = nn.ConvTranspose2d(channels, channels, (k1, k1), dilation=(0,0)) # local receptive field
 
        self.decode = nn.Sequential(self.rconv3, self.rconv2, self.rconv1)
...
class ReZero_base(nn.Module):
    ...
    def __init__(self, ntoken, channels, ninp, nhead, nhid, nlayers, dropout=0.5, initialization=None, activation='relu',spatial=False):
        ...
        encoder_layers = RZTXEncoderLayer(ninp, nhead, nhid, dropout, activation, batch_first=True, spatial=spatial, channels=channels) #batch_first=False 
        ...
        if spatial:
            self.CNNAuto = CNNAutoencoder(5,5,ntoken)
            self.encoder_c = self.CNNAuto.encode
            self.decoder_c = self.CNNAuto.decode
    ...
    def encoder1D(self, inpt):
        self.nc, self.sx, self.sy = inpt.shape[-3:]
    ...
    def decoder1D(self, outpt):
        ...
        outpt = outpt.reshape(outpt.shape[0],outpt.shape[1],self.nc,self.sx,self.sy)
        return outpt
    ...
    def encoder2D(self, inpt):
        ...
        return cnnpt.reshape(shape[0], shape[1], -1) # NCL
    ...
    def forward(self, src, has_mask=True):
        ...
        if self.spatial:
            shape_params = (self.cnnl,self.cnnx,self.cnny)
        else:
            shape_params = None# src = self.pos_encoder(src)
        output = self.transformer_encoder(src, shape_params)#, self.src_mask)
...
class RZTXEncoderLayer(Module):
    ...
    """
    def __init__(self, d_model, nhead, dim_feedforward=2048, dropout=0.1, activation='relu', batch_first=True, spatial=False, channels=1):
        super().__init__()
    ...
    def forward(self, src, shape_params, src_mask=None, src_key_padding_mask=None, is_causal=False):
        ...
        if self.spatial:
            shape = (src2.shape[0], src2.shape[1], shape_params[0], shape_params[1], shape_params[2])
            src3 = src2.reshape(shape)
            src4 = self.conv.seq(src3)
            src2 = src4.reshape(src2.shape)
        else:
            src2 = self.conv.seq(src2)
