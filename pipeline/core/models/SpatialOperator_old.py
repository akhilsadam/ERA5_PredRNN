...
from core.spatial_operators.MovingBasisOperator7W1 import Operator
...
class SpatialOperator(BaseModel):
    def __init__(self, num_layers, num_hidden, configs):
        ...
        assert 'accelerator' in configs.__dict__
        self.accelerator = configs.accelerator
        
        self.operator = Operator(self.in_channel, self.input_length, self.height, self.width, device=self.device, nlayers=1, activation = self.activation, accelerator = self.accelerator)
