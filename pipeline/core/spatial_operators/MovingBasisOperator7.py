...
class Operator(nn.Module):
    def __init__(self, nlatent, ntime, h, w, device, n_embd=400, nlayers=1, activation=torch.nn.ReLU(), **kwargs):
        ...
        self.in_cnn = DeformConv2d(1, self.nchan, kernel_size=self.k, stride=1, padding=1, bias=True) # identify momenta
        self.out_cnn = DeformConv2d(1, self.nchan, kernel_size=self.k, stride=1, padding=1, bias=False) # create shifts
        ...
        self.out_cnn.weight.data = nn.Parameter((1/self.nchan) * torch.tensor([[0,0,0],[0,1,0],[0,0,0]],device=device)[None,None,:,:].expand(self.nchan,1,self.k,self.k))
        # nn.init.xavier_uniform_(self.out_cnn.weight.data)
        # nn.init.constant_(self.out_cnn.bias.data,0.0)
        
        self.in_offset = nn.Parameter(torch.empty((2*self.k**2,self.h,self.w),device=device))
        self.out_offset = nn.Parameter(torch.empty((2*self.k**2,self.h,self.w),device=device))
        
        nn.init.constant_(self.in_offset.data,0.0)
        nn.init.constant_(self.out_offset.data,0.0)
    ...
    def step_modes(self, u):
        ...
        momenta = self.in_cnn(uv, self.in_offset) # B nchan h w
        # return torch.einsum('bchw -> bhw',momenta).reshape(u.shape)
        shifts = self.out_cnn(uv, self.out_offset)
