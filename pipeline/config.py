...
model_config = \
    {
        ...
        'reZeroCNN_LG':{
            ...
            'activation': 'relu', # activation function
            'optimizer' :  lambda x,y : Adam(x, lr=5e-5), # final_lr=0.1), #SGD(x, lr=0.4),#, momentum=0.1, nesterov=True), #ASGD(x,lr=100*y), # [None, Adam, ASGD,...]'
            'scheduler' : lambda x : CyclicLR(x, base_lr=5e-6, max_lr=2e-3, cycle_momentum=False, step_size_up=20),
            'batch_size': 1, # batch size
            'test_batch_size': 1, # batch size for testin
        }, 
        'reZeroTF_POD':{
...
model_config_toy = \
    {# note base learning rate is 1e-3 for all models (denoted by y in the optimizer)
        ...
        'GateOperator':{
            ...
            # 'n_embd': 100, # number of hidden units
            # 'activation': 'sin',
            'optimizer' :  lambda x,y : Adam(x, lr=1e-6), # final_lr=0.1), #SGD(x, lr=0.4),#, momentum=0.1, nesterov=True), #ASGD(x,lr=100*y), # [None, Adam, ASGD,...]'
            'scheduler' : lambda x : CyclicLR(x, base_lr=1e-6, max_lr=1e-6, cycle_momentum=False, step_size_up=20),
        ...
        'reZeroSROM':{
            ...
            'activation': 'relu', # activation function
            'optimizer' :  lambda x,y : Adam(x, lr=5e-5), # final_lr=0.1), #SGD(x, lr=0.4),#, momentum=0.1, nesterov=True), #ASGD(x,lr=100*y), # [None, Adam, ASGD,...]'
            'scheduler' : lambda x : CyclicLR(x, base_lr=5e-6, max_lr=5e-3, cycle_momentum=False, step_size_up=20),
            'batch_size': 16, # batch size
